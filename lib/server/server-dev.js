...
import pluginMD from './plugins/markdown/markdown.js';
...
export default class ServerDevelopment {
  ...
  async initialize() {
    ...
    await this.#server.register(pluginAdapters, { config: this.#config, logger: this.#logger });
    ...
    await this.#server.register(pluginMD, { config: this.#config, hierarchy: this.#hierarchy });
